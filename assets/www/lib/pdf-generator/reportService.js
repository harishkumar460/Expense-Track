...
//
 ...
 serviceModule.service('ReportSvc', ['$q', '$rootScope', '$timeout', 'ReportBuilderSvc','$window',
                               reportSvc]);
//
...
//
	 function reportSvc($q, $rootScope, $timeout, ReportBuilderSvc,$window) {
...
//
		function saveFile(pdfBlob) {
			...
			try {
				...
				var fileTargetPath=cordova.file.externalRootDirectory+'/ExpenseBackup';
				$window.resolveLocalFileSystemURL(fileTargetPath,gotFS,fail);
			}
			catch (e) {
			...
			function gotFS(fileSystem) {
				...
				fileSystem.getFile(ReportBuilderSvc.reportData.fileName+".pdf", {create: true, exclusive: false}, gotFileEntry, fail);
			}
